...
const DoctorDashboard = () => {
  const { dashData, getDashData, dToken } =
    useContext(DoctorContext);
    const { currency } = useContext(AppContext);
    const { slotDateFormat } = useContext(AppContext);
    const { cancelAppointment } = useContext(AppContext);
...
{
  dashData.latestAppointments && dashData.latestAppointments.map((item,key)=>(
    <div className='flex items-center px-6 py-3 gap-3 hover:bg-gray-100' key={key}>
